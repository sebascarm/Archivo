...
bool C_Archivo::Read_Line(string &Texto_Linea) {
	bool FindeLinea = false;
	Texto_Linea ="" ;	// Vaciado del textp
	int PosFin, Longitud;
	std::string TextoLinea = "";
	//Obtener la linea
	PosFin = (int)Contenido.find("\n", PosParam);
	...
	//Revisamos si hay datos en el texto
	if (Funciones::Texto_vacio(TextoLinea)) {
		Texto_Linea = "";			//Ponemos el primer vector vacio
		return true;				//Para forzar a leer otra linea (revisar de devolver el vector vacio)
	}

	// Almacenamos
	Texto_Linea = TextoLinea;
	Funciones::Clear_Text(Texto_Linea);
	return true;
}
